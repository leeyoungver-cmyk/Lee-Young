...
const ko = (
  <>
    <p>
      ...
      의심하는 것에서 작업을 시작한다.<br className="md:hidden" /> 오늘날 이미지는 기록, 증거, 경험의 대리물로
    ...
    <p>
      ...
      시간과 매체는 분리되지 않고 중첩되며, 만들어진 장면은 하나의 서사로 정리되지 않는다.
    </p>
    <p>
...
const en = (
  <>
    <p>
      ...
      have entered a state in which the actual and the generated have become increasingly difficult to discern.
    </p>
    <p>
...
export default function TextSection({ lang = 'ko' }: { lang?: Lang }) {
  return (
    <div className="px-8 md:px-14 lg:px-20 py-16 md:py-24 max-w-3xl">
      ...
      <div className={`mt-14 text-[13px] md:text-[15px] leading-[1.88] text-ink/88 space-y-7 ${lang === 'en' ? 'text-pretty' : 'break-keep'}`}>
        {lang === 'en' ? en : ko}
      </div>
    </div>
  );
