        ...
        <div className="md:hidden">
          ...
          <div className="px-5 h-12 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => setLang('ko')}
                className={`text-[10px] tracking-wider uppercase transition-all duration-200 ${lang === 'ko' ? 'text-ink font-medium' : 'text-muted hover:text-ink hover:[filter:blur(0.6px)] active:text-ink active:[filter:blur(0.6px)]'}`}
              >KR</button>
              <span className="text-muted text-[9px]">/</span>
              <button
                onClick={() => setLang('en')}
                className={`text-[10px] tracking-wider uppercase transition-all duration-200 ${lang === 'en' ? 'text-ink font-medium' : 'text-muted hover:text-ink hover:[filter:blur(0.6px)] active:text-ink active:[filter:blur(0.6px)]'}`}
              >EN</button>
            </div>
          </div>
          {/* Row 2: nav */}
            ...
            <div className="px-5 pb-4 pt-2 flex items-center justify-between">
              {years.map((year) => (
                <button
                  ...
                  className="text-[10px] tracking-wider tabular-nums text-muted hover:text-ink hover:[filter:blur(0.9px)] active:text-ink active:[filter:blur(0.9px)] transition-all duration-150"
                >
                  {year}
                </button>
              ))}
        ...
        <div className="hidden md:block">
          <div className="relative px-14 lg:px-20 h-16 flex items-center justify-between">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-1">
              <button
                onClick={() => setLang('ko')}
                className={`text-[11px] tracking-wider2 uppercase transition-all duration-200 ${
              ...
              {NAV.map((n) => (
                <div key={n.key} className={`relative flex justify-center w-20 ${n.key === 'works' ? 'group/works' : ''}`}>
                  ...
                  {n.key === 'works' && years.length > 0 && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-8 opacity-0 group-hover/works:opacity-100 pointer-events-none group-hover/works:pointer-events-auto transition-all duration-200 translate-y-1 group-hover/works:translate-y-0">
                      <div className="flex flex-col items-center gap-2.5">
                        {years.map((year) => (
                          <button
                            key={year}
                            onClick={() => goToYear(year)}
                            className="text-[10px] tracking-wider tabular-nums text-muted hover:text-ink hover:[filter:blur(0.9px)] active:text-ink active:[filter:blur(0.9px)] transition-all duration-150 whitespace-nowrap"
                          >{year}</button>
                        ))}
                      </div>
                    </div>
