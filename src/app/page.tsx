        ...
        <div className="md:hidden">
          ...
          <div className="px-5 h-12 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => setLang('ko')}
                className={`text-[10px] tracking-wider uppercase transition-all duration-200 ${lang === 'ko' ? 'text-ink font-medium' : 'text-muted hover:text-ink hover:[filter:blur(0.6px)]'}`}
              >KR</button>
              <span className="text-muted text-[9px]">/</span>
              <button
                onClick={() => setLang('en')}
                className={`text-[10px] tracking-wider uppercase transition-all duration-200 ${lang === 'en' ? 'text-ink font-medium' : 'text-muted hover:text-ink hover:[filter:blur(0.6px)]'}`}
              >EN</button>
            </div>
          </div>
          {/* Row 2: nav */}
              ...
              {NAV.map((n) => (
                <button
                  ...
                  className={`relative text-[10px] tracking-wider uppercase transition-all duration-200 pb-0.5 ${
                    active === n.key ? 'text-ink' : 'text-muted hover:text-ink hover:[filter:blur(0.6px)]'
                  }`}
                >
                  {n.label}
            ...
            <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-1">
              <button
                ...
                className={`text-[11px] tracking-wider2 uppercase transition-all duration-200 ${
                  lang === 'ko' ? 'text-ink font-medium' : 'text-muted hover:text-ink hover:[filter:blur(0.7px)]'
                }`}
              >KR</button>
              ...
              <button
                onClick={() => setLang('en')}
                className={`text-[11px] tracking-wider2 uppercase transition-all duration-200 ${
                  lang === 'en' ? 'text-ink font-medium' : 'text-muted hover:text-ink hover:[filter:blur(0.7px)]'
                }`}
              >EN</button>
            </div>
                ...
                <div key={n.key} className={`relative flex justify-center w-20 ${n.key === 'works' ? 'group/works' : ''}`}>
                  <button
                    ...
                    className={`relative text-[11px] tracking-wider uppercase transition-all duration-200 pb-1 ${
                      active === n.key ? 'text-ink' : 'text-muted hover:text-ink hover:[filter:blur(0.7px)]'
                    }`}
                  >
                    {n.label}
                    <span className={`absolute bottom-0 left-0 right-0 h-px bg-ink transition-transform duration-300 origin-left ${
